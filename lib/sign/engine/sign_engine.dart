 ...
 {
   final  JsonRpcInteractorInterface jsonRpcInteractor;
   final  GetPendingRequestsUseCase getPendingRequestsUseCase;
   final  KeyManagementRepository crypto;
   final  SessionStorageRepository sessionStorageRepository;
   final  MetadataStorageRepositoryInterface metadataStorageRepository;
   final  PairingInterface pairingInterface;
   final  PairingControllerInterface pairingHandler;
   ...
   final  Logger logger;
    ...
    fun setup() {
        jsonRpcInteractor.wsConnectionFailedFlow.onEach { walletConnectException ->
            when (walletConnectException) {
                is ProjectIdDoesNotExistException, is InvalidProjectIdException -> _engineEvent.emitData(ConnectionState(false, walletConnectException))
                else -> _engineEvent.emitData(SDKError(InternalError(walletConnectException)))
            }
        }.launchIn(scope)

        jsonRpcInteractor.isConnectionAvailable
            .onEach { isAvailable -> _engineEvent.emitData(ConnectionState(isAvailable)) }
    ...
    private fun collectInternalErrors(): Job =
        merge(jsonRpcInteractor.internalErrors, pairingHandler.findWrongMethodsFlow)
            .onEach { exception -> _engineEvent.emitData(SDKError(exception)) }
            .launchIn(scope)
    ...
    private fun onSessionPropose(request: WCRequest, payloadParams: SignParams.SessionProposeParams) {
        ...
        try {
            ...
            scope.launch { _engineEvent.emitData(payloadParams.toEngineDO()) }
        } catch (e: Exception) {
            jsonRpcInteractor.respondWithError(
                request,
    ...
    private fun onSessionSettle(request: WCRequest, settleParams: SignParams.SessionSettleParams) {
        ...
        try {
            ...
            scope.launch { _engineEvent.emitData(session.toSessionApproved()) }
        } catch (e: SQLiteException) {
    ...
    private fun onSessionDelete(request: WCRequest, params: SignParams.DeleteParams) {
        ...
        try {
            ...
            scope.launch { _engineEvent.emitData(params.toEngineDO(request.topic)) }
        } catch (e: Exception) {
            jsonRpcInteractor.respondWithError(
                request,
    ...
    private fun onSessionRequest(request: WCRequest, params: SignParams.SessionRequestParams) {
        ...
        try {
            ...
            scope.launch { _engineEvent.emitData(params.toEngineDO(request, sessionPeerAppMetaData)) }
        } catch (e: Exception) {
            jsonRpcInteractor.respondWithError(
                request,
    ...
    private fun onSessionUpdate(request: WCRequest, params: SignParams.UpdateNamespacesParams) {
        ...
        try {
            ...
            sessionStorageRepository.deleteNamespaceAndInsertNewNamespace(session.topic.value, params.namespaces, request.id, onSuccess = {
                ...
                scope.launch {
                    _engineEvent.emitData(EngineDO.SessionUpdateNamespaces(request.topic, params.namespaces.toMapOfEngineNamespacesSession()))
                }
            }, onFailure = {
                jsonRpcInteractor.respondWithError(
                    request,
    ...
    private fun onSessionExtend(request: WCRequest, requestParams: SignParams.ExtendParams) {
        ...
        try {
            ...
            jsonRpcInteractor.respondWithSuccess(request, irnParams)
            scope.launch { _engineEvent.emitData(session.toEngineDOSessionExtend(Expiry(newExpiry))) }
        } catch (e: Exception) {
            jsonRpcInteractor.respondWithError(
                request,
                Uncategorized.GenericError("Cannot update a session: ${e.message}, topic: ${request.topic}"),
            ...
            when (val response = wcResponse.response) {
                is JsonRpcResponse.JsonRpcResult -> {
                    ...
                    jsonRpcInteractor.subscribe(sessionTopic) { error ->
                        scope.launch {
                            _engineEvent.emitData(SDKError(InternalError(error)))
                        }
                    }
                }
                is JsonRpcResponse.JsonRpcError -> {
                    logger.log("Session proposal reject received: ${response.error}")
                    scope.launch { _engineEvent.emitData(EngineDO.SessionRejected(pairingTopic.value, response.errorMessage)) }
                }
            }
        } catch (e: Exception) {
            scope.launch { _engineEvent.emitData(SDKError(InternalError(e))) }
        }
    }
        ...
        try {
            ...
            when (wcResponse.response) {
                is JsonRpcResponse.JsonRpcResult -> {
                    ...
                    scope.launch { _engineEvent.emitData(EngineDO.SettledSessionResponse.Result(session.toEngineDO())) }
                }
                is JsonRpcResponse.JsonRpcError -> {
            ...
            when (val response = wcResponse.response) {
                is JsonRpcResponse.JsonRpcResult -> {
                    ...
                    sessionStorageRepository.deleteNamespaceAndInsertNewNamespace(session.topic.value, namespaces, responseId,
                        ...
                        onFailure = {
                            scope.launch { _engineEvent.emitData(EngineDO.SessionUpdateNamespacesResponse.Error("Unable to update the session")) }
                        })
                }
                is JsonRpcResponse.JsonRpcError -> {
                    logger.error("Peer failed to update session namespaces: ${response.error}")
                    scope.launch { _engineEvent.emitData(EngineDO.SessionUpdateNamespacesResponse.Error(response.errorMessage)) }
                }
            }
        } catch (e: Exception) {
            scope.launch { _engineEvent.emitData(SDKError(InternalError(e))) }
    ...
    private fun onSessionRequestResponse(response: WCResponse, params: SignParams.SessionRequestParams) {
        try {
            ...
            val method = params.request.method
            scope.launch { _engineEvent.emitData(EngineDO.SessionPayloadResponse(response.topic.value, params.chainId, method, result)) }
        } catch (e: Exception) {
            scope.launch { _engineEvent.emitData(SDKError(InternalError(e))) }
        }
    }
